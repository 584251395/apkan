...
public class DaoSupport<T> implements BaseDao<T>{
	 ...
	 * 批量存入数据库
	 */
	public void saveList(List<T> obj) {
		Session session=SessionFactoryUtils.getNewSession(this.template.getSessionFactory());
	...
	@Transactional(propagation=Propagation.NOT_SUPPORTED,readOnly=true)
	public PageModel<T> find(int pageNo, int maxResult,final String select, String where,
			Object[] queryParams) {
		return find(select,where, queryParams, null, pageNo, maxResult);
	}
	/**
	 * 分页查询
	 * @param select select 参数
	 * @param where 查询条件
	 ...
	 */
	@Transactional(propagation=Propagation.NOT_SUPPORTED,readOnly=true)
	public PageModel<T> find(final String select,final String where, final Object[] queryParams,
		...
		getTemplate().execute(new HibernateCallback() {//执行内部方法
			public Object doInHibernate(Session session)
				...
				String hql = new StringBuffer().append(select)//select语句
						         .append("from ")//添加form字段
